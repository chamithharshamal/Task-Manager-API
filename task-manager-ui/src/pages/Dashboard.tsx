...
export const Dashboard: React.FC = () => {
    ...
    const [showDueThisWeek, setShowDueThisWeek] = React.useState(false);
    ...
    const { data: tasksData = [], isLoading, isError } = useQuery({
        queryKey: ['tasks', search, showDueThisWeek],
        queryFn: () => {
            if (search.trim()) return taskService.searchTasks(search);
            if (showDueThisWeek) return taskService.getTasksDueThisWeek();
            return taskService.getTasks();
        },
        staleTime: 1000 * 60 * 5,
    });

    // Handle both cases where backend might return List or Paginated object (though here it's List)
    const tasks = Array.isArray(tasksData) ? tasksData : (tasksData as any).content || [];
    ...
    const stats = {
        total: tasks.length,
        todo: tasks.filter((t: Task) => t.status === 'TO_DO').length,
        inProgress: tasks.filter((t: Task) => t.status === 'IN_PROGRESS').length,
        completed: tasks.filter((t: Task) => t.status === 'COMPLETED').length,
    };

    const filteredTasks = tasks; // Filtering is now handled by backend queryFn
                        ...
                        <div className="glass-card overflow-hidden">
                            <div className="p-6 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div className="flex flex-wrap items-center gap-4">
                                    <button
                                        onClick={() => {
                                            setShowDueThisWeek(!showDueThisWeek);
                                            if (!showDueThisWeek) setSearch('');
                                        }}
                                        className={cn(
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (e.target.value) setShowDueThisWeek(false);
                                            }}
